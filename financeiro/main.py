...
def adicionar():
    total_receita = db.session.query(func.sum(Receita.valor)).filter_by(usuario_id=session['id']).scalar()
    total_despesa = db.session.query(func.sum(Despesa.valor)).filter_by(usuario_id=session['id']).scalar()
    receitas = Receita.query.filter_by(usuario_id=session['id'])

    return render_template('receitas.html', total_receita=total_receita, total_despesa=total_despesa, receitas=receitas)
...
def adicionardespesa():
    total_receita = db.session.query(func.sum(Receita.valor)).filter_by(usuario_id=session['id']).scalar()
    total_despesa = db.session.query(func.sum(Despesa.valor)).filter_by(usuario_id=session['id']).scalar()
    despesas = Despesa.query.filter_by(usuario_id=session['id'])

    return render_template('despesas.html', total_receita=total_receita, total_despesa=total_despesa, despesas = despesas)
...
def principal():
    ...
    return render_template('principal.html', total_receita=total_receita, total_despesa=total_despesa, despesas=despesas, receitas=receitas)
